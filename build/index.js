...
function format_linking_api_cloning_url(report_id, report_name, project_id, dataset_id, datasources) {
...
async function create_linking_api_cloning_url(answers, project_id) {
    ...
    const dashboard_url = format_linking_api_cloning_url(dash_answers.dashboard_id.trim(), dash_answers.dashboard_name.trim(), project_id, answers.dashboard_dataset, datasources);
    console.log('Open the following link in the browser for cloning the dashboard:');
    console.log(chalk.cyanBright(dashboard_url));
    return dashboard_url;
}
async function get_gcp_project(answers) {
    var _a;
    ...
    spinner.start();
    const cp = spawn('gcloud projects list --format="csv(projectId,name)" --sort-by=projectId --limit=500', [], {
        shell: true,
        // inherit stdin, and wrap stdout/stderr
        stdio: ['inherit', 'pipe', 'inherit'],
    });
    let projects_csv = '';
    (_a = cp.stdout) === null || _a === void 0 ? void 0 : _a.on('data', chunk => {
        projects_csv += chunk;
    });
